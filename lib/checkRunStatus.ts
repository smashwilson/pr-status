...
export type CheckStatusState = 'QUEUED' | 'IN_PROGRESS' | 'COMPLETED' | 'WAITING' | 'PENDING' | 'REQUESTED';
...
const FAILURE_CONCLUSIONS: Set<CheckConclusionState> = new Set(
  ['TIMED_OUT', 'CANCELLED', 'FAILURE', 'STARTUP_FAILURE'],
);
...
export class CheckRunStatus extends Status {
  ...
  constructor(url: string, suiteName: string, runName: string, status: CheckStatusState, conclusion: CheckConclusionState) {
    super(url);
